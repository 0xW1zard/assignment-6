    ...
    <div>
      <div className={`card w-96 bg-base-100 shadow-sm h-full ${isPopular ? "bg-blue-500 text-white" : ""}`}>
        <div className="card-body relative">
         {
            isPopular && ( <span className="badge badge-md rounded-full badge-warning absolute -top-2 right-38">Most Popular</span>)
         }
          <div className="">
            <h2 className="text-3xl font-bold">{plan.planName}</h2>
            <p className={isPopular ? "text-white" : "text-gray-600"}>{tagline}</p>
          </div>
          <h2 className="text-5xl mt-2">{plan.price}<span className="text-xl">/Month</span></h2>
